...
class AnalysisBox
{
	...
	public:
		...
		Int_t Batch_MyId() const {return(an->Batch_MyId());} 
		Int_t Batch_NumJobs() const {return(an->Batch_NumJobs());} 
		...
		Muon Muons(UInt_t n, int correction = 0) const {return(an->Muons(n, correction));}
		UInt_t NumMuons() const {return(an->NumMuons());}

		Electron Electrons(UInt_t n, int correction = 0) const {return(an->Electrons(n, correction));}
		...
		Vertex PrimVertices(UInt_t n) const {return(an->PrimVertices(n));}
		...
		Double_t GetPileUpMaxWeight(vector<Double_t>& datadist) const {return(an->GetPileUpMaxWeight(datadist));}
		Double_t GetPileUpWeight(vector<Double_t>& datadist) const {return(an->GetPileUpWeight(datadist));}
		Double_t GetPrimVertexMaxWeight(vector<Double_t>& datadist) const {return(an->GetPrimVertexMaxWeight(datadist));}
		...
		Int_t IsLumiAvailable() const {return(an->IsLumiAvailable());}
		Double_t GetInstLumi() const {return(an->GetInstLumi());}
		Double_t GetAvgPU() const {return(an->GetAvgPU());}
		Double_t GetLumi(Int_t format = 0) {return(an->GetLumi(format));}
		//Double_t GetLumiBlockLumi() {return(an->GetLumiBlockLumi());}
		bool IsData() const {return(an->IsData());}
		bool IsMC() const {return(an->IsMC());}
};
