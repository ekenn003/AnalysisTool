...
class Muon : public TLorentzVector, public TriggerObject
{
	...
	public:
		...
		bool IsPFMuon() const {return((type & 1<<6) > 0);}
...
class Vertex : public TVector3
{
	...
	public:
		...
		Double_t YError() const {return(Sqrt(covmatrix(1,1)));}
		Double_t ZError() const {return(Sqrt(covmatrix(2,2)));}
		bool IsFake() const {return(info & 1<<0);}
		bool IsValid() const {return(info & 1<<1);}
		TMatrixFSym CovMatrix() const {return(covmatrix);}
		Double_t CovMatrix(Int_t i, Int_t j) const {return(covmatrix(i,j));}
...
class Jet : public TLorentzVector
{
	...
	public:
		...
		Float_t simpleSecondaryVertexHighPurBJetTags() const {return(btag[2]);}
		Float_t simpleSecondaryVertexHighEffBJetTags() const {return(btag[3]);}
		Float_t combinedSecondaryVertexBJetTags() const {return(btag[4]);}
		Float_t combinedSecondaryVertexMVABJetTags() const {return(btag[5]);}
...
class Luminosity
{
	...
	public:
		Luminosity(Float_t Lumival, Float_t Lumierr, Float_t Livefraction, Float_t Deadfraction, Float_t Avgpu, UInt_t Quality, UInt_t Numevents, UInt_t Numeventsprocessed, UInt_t Hlttable, UInt_t L1techtable, UInt_t L1algotable, vector<string> newfiles) : lumival(Lumival), lumierr(Lumierr), livefraction(Livefraction), deadfraction(Deadfraction), avgpu(Avgpu), quality(Quality), numevents(Numevents), numeventsorig(Numeventsprocessed), hlttable(Hlttable), l1techtable(L1techtable), l1algotable(L1algotable), counter(0) {AddFiles(newfiles);}
		...
		const vector<string>& GetFiles() const {return(filenames);}
		string GetFilesString() const;
		Float_t LumiValue() const {return(lumival);}
		Float_t AvgPU() const {return(avgpu);}
		...
		void AvgPU(Float_t myavgpu){avgpu = myavgpu;}
